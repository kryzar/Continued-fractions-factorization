...
Posons pour commencer quelques définitions qui seront utiles pour décrire le code.
...
forme sous laquelle nous collectons les données. A l'issue de la première
phase, on espère avoir collecté \texttt{nb\_want\_AQp} paires $(A,Q)$ avec
$Q_n$ friable\footnote{ou résultant de la \og large prime variation \fg{} mais 
...
Décrivons à présent la phase de collecte des données. Le nombre de paires $(A,Q)$
collectées sera stockée dans le champ texttt{nb\_AQp} de la structure 
\texttt{Results}. Concernant les vecteurs
historiques \texttt{exp\_vect}, il suffit d'initialiser \texttt{exp\_vects[i]} 

évoquer à un moment \texttt{init\_hist\_vects}. Le reste demande un peu plus
d'explications \\ 
