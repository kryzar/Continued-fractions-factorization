...
\KwIn{tableau $EXP\_VECTS[0 \cdots nb\_AQp -1 ] $ des vecteurs exposants 
         tableau $HIST\_VECTS[0 \cdots nb\_AQp -1 ]$ des vecteurs historiques}
\vspace{0.5em}
\KwOut{$HIST\_VECTS[0 \cdots nb\_AQp -1]$ après le pivot, le nombre
        $nb\_lin\_rel$ de relations linéaires trouvées, $LIN\_REL\_IND[0 \cdots
        nb\_lin\_rel-1]$ contenant les indices des lignes où une relation 
        linéaire a été trouvée}
\vspace{0.5em}

    \tcc{tableau pour stocker les indices des bits de poids fort des vecteurs 
     exposants}
créer tableau $MSB\_IND[0 \cdots nb\_AQp - 1]$\;  
créer tableau $LIN\_REL\_IND$\; 
$nb\_lin\_rel \gets 0$\; 
    \tcc{Initialisation du tableau $MSB\_IND$ : \textsc MSB(x) renvoie $0$
    si x est nul, l'indice du bit (numéroté à partir de $1$) de poids fort
    de x sinon.
}\
\For{$i \gets 0 \textbf{ à } nb\_AQp -1 $ }{
    $MSB\_IND[i] \gets \textsc{Msb} (EXP\_VECTS[i])$  \; 
}
    \For{$ j \gets \textsc{Max}(MSB\_IND)$ $\textbf{à } 1 $ }{
    $pivot \gets 
    \begin{cases}
        \min \big\{ i \in  \{ 0, nb\_AQp - 1 \} \big\vert MSB\_IND[i] = j \big\}\\
        \emptyset \text{ si pour tout }i \in \{ 0, nb\_AQp - 1 \}, \quad
        MSB\_IND[i] \neq j \big\} 
   \end{cases}
    $\;
    \If{$pivot \neq \emptyset$}{
        \For{$i \gets pivot + 1 \textbf{ à } nb\_AQp -1 $}{
            \If{$MSN\_IND[i] = j$}{
                $EXP\_VECTS[i] \gets EXP\_VECTS[i] \oplus EXP\_VECTS[pivot]$ \; 
                $HIST\_VECTS[i] \gets  HIST\_VECTS[i] \oplus HIST\_VECTS[pivot]$\; 
                $MSB\_IND[i] \gets $ \textsc{Msb}$(EXP\_VECTS[i])$\; 
                \If{$EXP\_VECTS[i] = 0 $}{
                    ajouter $i$ au tableau $LIN\_REL\_IND$\; 
...
\Return{$EXP\_VECTS[0 \cdots nb\_AQp -1 ]$, $lin\_rel\_indexes$}, 
    $LIN\_REL\_IND[0 \cdots nb\_lin\_rel-1]$ \;
